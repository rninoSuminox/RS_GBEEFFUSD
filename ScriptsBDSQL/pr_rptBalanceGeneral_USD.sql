...
--EJECUTA SP, PARA LLENAR TABLA CON costos y aplicar filtros
	...
	DECLARE @MONFUNC VARCHAR(6) = (SELECT TOP 1 RTRIM(CURNCYID) CURNCYID FROM TEMPSALDOSF WHERE CURNCYID<>'USD' order by 1); --MONEDA BSF
	...
	CTE_CTASFULL AS ( --completa periodos y cuentas
	           SELECT A.ANIO,A.PERIODO,A.PERIODO MES,CONVERT(DATE,CONCAT(A.ANIO,RIGHT(CONCAT('00',A.PERIODO),2),'01')) TRXDATE
				...
				INNER JOIN (SELECT DISTINCT CURNCYID FROM TEMPSALDOSF WHERE LEFT(CURNCYID,1) IN ('U','B')) M ON 1=1
				...
				OR LEFT(C.ACTNUMST,8)='311-04-3'),--cuenta de superavit de revaluacion lleva dolares manuales 070722
	CTE_CUENTAS AS (
				SELECT ANIO,PERIODID,PERIODID MES,TRXDATE,ACTNUMST						
						,CASE WHEN SUBSTRING(ACTNUMST,8,1) IN ('6','7','8') THEN 'USD'
							  WHEN NOTA=1 THEN 'USD' --PARA CONTROLAR LOS ASIENTOS CON DOLARES ASIGNADOS 04/07/22
	 ...
	 --datos principales del reporte
	 	 CTE_AGRUPA AS (
	 		SELECT ANIO,MES,MAX(YEAR(TRXDATE)) ANIOTRX,ACTNUMST						
				...
				,SUM(sum(iif(CURNCYID='USD' AND NOTA=0,DEBITAMT-CRDTAMNT,0))) 
							OVER(PARTITION BY actnumst,CURNCYID ORDER BY ANIO,MES,CURNCYID) SALDOUSD
				,SUM(sum(iif(CURNCYID='USD' AND NOTA=1,ORDBTAMT-ORCRDAMT,0)))  --DOLARES MANUALES
							OVER(PARTITION BY actnumst,CURNCYID ORDER BY ANIO,MES,CURNCYID) MANUALUSD
		...
		CTE_INICIAL AS (SELECT ANIO,MES,ANIOTRX,ACTNUMST,CURNCYID
								,CASE WHEN @MONEDA= 'USD' and CURNCYID= 'USD' then 'Moneda Extranjera'
								 ...
								 END  TXTMONEDA
										...
										,ISNULL(MANUALUSD,0) MANUALUSD --dolares manuales
						 FROM CTE_FILTRA),
		 CTE_SALDOS AS ( --SALDO CTAS por mes
						 SELECT ANIO,MES,ANIOTRX,ACTNUMST
								,IIF(LEFT(ACTNUMST,1) IN ('3','2'),
									(ISNULL(SALDOBSF,0)+ISNULL(SALDOUSD,0)+MANUALUSD) * -1 --CAMBIO 20220610 ROBERTO
									,ISNULL(SALDOBSF,0)+ISNULL(SALDOUSD,0)+MANUALUSD) SALDO
								,CURNCYID
								,TXTMONEDA
						   FROM CTE_INICIAL
